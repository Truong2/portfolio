      ...
      <div className="relative h-[650px] w-[510px] overflow-hidden px-10 py-10 text-left text-white">
        ...
        <div className="relative z-10 flex h-full flex-col">
          ...
          <div className="mt-7 flex flex-wrap gap-2">
            {["React", "Next.js", "Vue", "Nuxt", "TypeScript", "OpenLayers"].map((item) => (
              <span key={item} className="rounded-lg border border-cyan-300/14 bg-cyan-300/[0.045] px-2.5 py-1.5 text-[9px] font-medium text-cyan-100/85">{item}</span>
            ))}
          </div>
...
function PaperStack() {
  return <group>{Array.from({ length: 8 }, (_, index) => <mesh key={index} position={[HALF_PAGE + 0.02, 0, -0.115 + index * 0.022]}><boxGeometry args={[PAGE_WIDTH - 0.08 - index * 0.004, PAGE_HEIGHT - 0.09 - index * 0.005, 0.018]} /><meshStandardMaterial color={index % 2 === 0 ? "#d9e3ee" : "#cbd8e5"} roughness={0.88} /></mesh>)}</group>;
}
...
function CurvedTurningPage({ currentSpread, direction, reducedMotion }: { currentSpread: number; direction: -1 | 0 | 1; reducedMotion: boolean }) {
  ...
  const fromSpread = React.useRef(currentSpread);
  ...
  useFrame((_, delta) => {
    if (previousSpread.current !== currentSpread) {
      fromSpread.current = previousSpread.current;
      previousSpread.current = currentSpread;
      progress.current = reducedMotion || (fromSpread.current === 0 && currentSpread === 1) ? 1 : 0;
      delay.current = fromSpread.current === 0 && currentSpread > 1 && !reducedMotion ? 0.42 : 0;
    }

    const mesh = meshRef.current;
    if (!mesh) return;
    if (delay.current > 0) {
      delay.current = Math.max(0, delay.current - delta);
      mesh.visible = false;
      return;
    }
  ...
  return <mesh ref={meshRef} geometry={geometry} position={[0, 0.02, 0.28]} castShadow><meshPhysicalMaterial color="#d7e2ec" side={THREE.DoubleSide} roughness={0.74} metalness={0.02} clearcoat={0.12} emissive="#6d83a4" emissiveIntensity={0.025} /></mesh>;
}
...
function BookModel({ reducedMotion }: { reducedMotion: boolean }) {
  ...
  return (
    <group ref={rootRef} position={[-HALF_PAGE, -0.16, 0]} rotation={[-0.035, -0.13, -0.015]}>
      <Hardcover position={[HALF_PAGE, 0, -0.22]} />
      <PaperStack />
      <group position={[HALF_PAGE, 0, 0.13]}>
        {open && <PageFrame><SpreadPageContent index={currentSpread} side="right" /></PageFrame>}
      </group>
      ...
      <group ref={frontCoverRef} position={[0, 0, 0.19]}>
        <group position={[HALF_PAGE, 0, 0]} onClick={() => { if (!open) nextSpread(); }}>
          ...
          {!open && <CoverContent />}
          <group position={[0, 0, -0.205]} rotation={[0, Math.PI, 0]}>
            {open && <PageFrame><SpreadPageContent index={currentSpread} side="left" /></PageFrame>}
          </group>
        </group>
      </group>

      <mesh position={[0, 0, -0.02]}>
        <boxGeometry args={[0.18, PAGE_HEIGHT + 0.08, 0.44]} />
        <meshPhysicalMaterial color="#062746" roughness={0.28} metalness={0.52} clearcoat={0.8} emissive="#22d3ee" emissiveIntensity={0.26} />
      </mesh>
      ...
      <mesh position={[0, -PAGE_HEIGHT / 2 - 0.28, -0.08]} rotation={[0, 0, 0.04]}>
        <boxGeometry args={[0.08, 0.72, 0.025]} />
        <meshStandardMaterial color="#7c3aed" emissive="#7c3aed" emissiveIntensity={0.28} />
      </mesh>
    </group>
  );
}
...
function HolographicStage({ lightMode }: { lightMode: boolean }) {
  return (
    <group position={[0, -3.0, -0.25]}>
      <gridHelper args={[17, 34, lightMode ? "#7da4c8" : "#173f68", lightMode ? "#bed1e5" : "#0b213b"]} position={[0, -0.03, 0]} />
      {[2.25, 3.1, 4.05, 5.05].map((radius, index) => <mesh key={radius} rotation={[Math.PI / 2, 0, index * 0.16]}><torusGeometry args={[radius, index === 1 ? 0.018 : 0.009, 8, 160]} /><meshBasicMaterial color={index % 2 ? "#8b5cf6" : "#22d3ee"} transparent opacity={lightMode ? 0.16 : 0.5 - index * 0.075} /></mesh>)}
...
export function SpatialCvScene({ reducedMotion, lightMode }: { reducedMotion: boolean; lightMode: boolean }) {
  ...
  return (
    <>
      ...
      {!lightMode && <Stars radius={18} depth={12} count={260} factor={2.1} saturation={0} fade speed={0.22} />}
